...
public interface UserPostRepository extends JpaRepository<UserPost, UUID>, JpaSpecificationExecutor<UserPost> {
    ...
    @Query("SELECT p FROM UserPost p WHERE p.author.userId = :userId ORDER BY p.createdAt DESC")
    Page<UserPost> findByUserIdOrderByCreatedAtDesc(@Param("userId") UUID userId, Pageable pageable);

    // Пошук за топіком/категорією
    @Query("SELECT p FROM UserPost p WHERE p.topic.postTopicId = :topicId")
    Page<UserPost> findByPostTagTopicId(@Param("topicId") UUID topicId, Pageable pageable);

    // Пошук за топіком з сортуванням
    @Query("SELECT p FROM UserPost p WHERE p.topic.postTopicId = :topicId ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM UserPost p WHERE " +
            ...
            "(:topicId IS NULL OR p.topic.postTopicId = :topicId)")
    ...
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM UserPost p " +
            "WHERE p.userPostId = :postId AND p.author.userId = :userId")
    ...
    @Query("SELECT COUNT(p) FROM UserPost p WHERE p.author.userId = :userId")
    ...
    @Query("SELECT p FROM UserPost p WHERE " +
            "(:userId IS NULL OR p.author.userId = :userId) AND " +
            "(:topicId IS NULL OR p.topic.postTopicId = :topicId) AND " +
    ...
    @Query("SELECT p FROM UserPost p WHERE p.author.userId = :userId AND p.topic.postTopicId = :topicId")
